...
{
	uint8_t Local_u8ErrorState = OK , Local_u8FlashStatus ;
	...
	if(Local_u8ErrorState == FLASH_IS_READY)
	{
		...
		if(Copy_FlashErase->EraseType == FLASH_SECTOR_ERASE)
		{
			...
			for(uint8_t Local_u8Sector=Copy_FlashErase->SartSector ; Local_u8Sector < Copy_FlashErase->EndSector ; Local_u8Sector++)
			{
				/* Wait until the flash is ready */
				Local_u8FlashStatus=FLASH_u8IsBusy();
...
uint8_t FLASH_u8Program(Flash_Write_TypeDef *Copy_FlashProgram)
{
	...
	if(Local_u8ErrorState == FLASH_IS_READY)
	{
		...
		case FLASH_PAR_DWORD : *(volatile uint32_t*)Copy_FlashProgram->BaseAddress = (uint32_t) Copy_FlashProgram->Data ;
				*(volatile uint32_t*)(Copy_FlashProgram->BaseAddress+4) = (uint32_t)( Copy_FlashProgram->Data >>32);  break;
